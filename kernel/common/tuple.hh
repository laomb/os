...
template <typename T, typename... Ts> class tuple<T, Ts...> : private tuple<Ts...> {
	using Base = tuple<Ts...>;
	T m_value;
  ...
  public:
	...
	constexpr explicit tuple(const T& t, const Ts&... ts) : Base(ts...), m_value(t) {}
...
template <usize I, typename... Ts> constexpr decltype(auto) get(tuple<Ts...>& t) {
	return t.template get<I>();
}
template <usize I, typename... Ts> constexpr decltype(auto) get(tuple<Ts...> const& t) {
	return t.template get<I>();
}
template <usize I, typename... Ts> constexpr decltype(auto) get(tuple<Ts...>&& t) {
...
struct tuple_size<tuple<Ts...>> : detail::integral_constant<usize, sizeof...(Ts)> {};
template <typename Tuple> static constexpr usize tuple_size_v = tuple_size<Tuple>::value;
